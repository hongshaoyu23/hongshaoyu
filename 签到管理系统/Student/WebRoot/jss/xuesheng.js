layui.extend({
    ...
    var tableIns = table.render({
        ...
        cols : [[
            ...
            {field: 'bname', title: '班级名',  align:'center'},
	    ...
	    switch(obj.event){
	      ...
	      case 'deddddlUser':	//ɾ���û�
	    	  if(data.length == 0 || data.length > 1){
					...
					layer.confirm('确定删除学生吗', {icon: 3, title:'提示ʾ'}, function(index){
    ...
    function upUser(userid){
    	...
    	layui.layer.open({
    		title : "学生修改",
    		type : 2,
    		content : "/Student/jsp/xuesheng/xueshengxiugai.jsp",
    		area:['400px','540px'],
    		success:function(layero, index){
    			$.ajax({
    				url:"/Student/news?action=xueshengxiugaichaxun",
    				type:"post",
    				data:{"userid":userid},
    				success:function(data){
    					...
    					body.find("#name").val(info.data.xname);
    					body.find("#loginname").val(info.data.loginname);
    ...
    function deddddlUser(userid){
    	$.ajax({
    		url:"/Student/news?action=xueshengdel",
    		data:{"userid":userid},
    		type:"post",
    		success:function(data){
    			if(data == 1){
...
/*    //�����ɫ
    ...
    function addUser(){
    	layui.layer.open({
    		title : "新增学生",
    		type : 2,
    		content : "/Student/jsp/xuesheng/xueshengadd.jsp",
    		area:['400px','490px'],
    	})
    }
